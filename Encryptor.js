...
class Encryptor {
    ...
    checkFileEncryption (data) {
        try {
            const parsed = cfb.parse(data);
            let encryptionInfoBuffer = _.find(parsed.FileIndex, { name: "EncryptionInfo" });
            let encryptedPackageBuffer = _.find(parsed.FileIndex, { name: "EncryptedPackage" });
            if (encryptionInfoBuffer && encryptedPackageBuffer) {
                return true;
            }
        } catch (e) {
        }
